...
export default async function OrderReceiptPage({
  ...
  return (
    <div className="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm print:border-0 print:shadow-none dark:border-slate-700 dark:bg-slate-800">
      ...
      <table className="w-full border-y border-slate-200 text-sm">
        <thead>
          ...
          {order.items.map((item) => (
            <tr key={item.id} className="border-t border-slate-100">
              ...
              <td className="text-right">
                {formatCurrency(Number(item.subtotal) + Number(item.taxAmount))}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className="ml-auto mt-5 max-w-xs space-y-2 text-sm">
        ...
        <p className="flex justify-between border-t border-slate-200 pt-2 text-lg font-bold">
          <span>Total</span>
      ...
      <div className="mt-6 border-t border-slate-200 pt-4 text-sm">
        ...
        {order.payments.map((payment) => (
          <p key={payment.id} className="mt-1 text-slate-500">
